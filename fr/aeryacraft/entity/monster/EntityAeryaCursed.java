package fr.aeryacraft.entity.monster;
...
public class EntityAeryaCursed extends EntityMob {

	protected static final IAttribute field_110186_bp = (new RangedAttribute((IAttribute) null,
			"aeryacursed.spawnReinforcements", 0.0D, 0.0D, 1.0D)).setDescription("Spawn Reinforcements Chance");
	...
	public EntityAeryaCursed(World worldIn) {
		super(worldIn);
	...
	public boolean attackEntityFrom(DamageSource source, float amount) {
		if (super.attackEntityFrom(source, amount)) {
			...
			if (var3 != null && this.worldObj.getDifficulty() == EnumDifficulty.HARD
				...
				EntityAeryaCursed var7 = new EntityAeryaCursed(this.worldObj);
					...
					if (World.doesBlockHaveSolidTopSurface(this.worldObj, new BlockPos(var9, var10 - 1, var11))
						...
						if (!this.worldObj.func_175636_b((double) var9, (double) var10, (double) var11, 7.0D)
							...
							this.getEntityAttribute(field_110186_bp).applyModifier(new AttributeModifier(
									"AeryaCursed reinforcement caller charge", -0.05000000074505806D, 0));
							var7.getEntityAttribute(field_110186_bp).applyModifier(new AttributeModifier(
									"AeryaCursed reinforcement callee charge", -0.05000000074505806D, 0));
							break;
						}
					}
				}
	...
	protected String getDeathSound() {
		return "mob.aeryacursed.death";
	}

	protected void func_180429_a(BlockPos p_180429_1_, Block p_180429_2_) {
		this.playSound("mob.aeryacursed.step", 0.15F, 1.0F);
	}

	protected Item getDropItem() {
		return AeryaItems.AVENTURINE_NUGGET;
	}
	...
	public IEntityLivingData func_180482_a(DifficultyInstance p_180482_1_, IEntityLivingData p_180482_2_) {
		...
		if (p_180482_2_1 == null) {
			p_180482_2_1 = new EntityAeryaCursed.GroupData(this.worldObj.rand.nextFloat() < 0.05F,
					this.worldObj.rand.nextFloat() < 0.05F, null);
		}

		if (p_180482_2_1 instanceof EntityAeryaCursed.GroupData) {
			EntityAeryaCursed.GroupData var4 = (EntityAeryaCursed.GroupData) p_180482_2_1;
		...
		if (var9 > 1.0D) {
			this.getEntityAttribute(SharedMonsterAttributes.followRange)
					.applyModifier(new AttributeModifier("Random aeryacursed-spawn bonus", var9, 2));
		}

		if (this.rand.nextFloat() < var3 * 0.05F) {
			this.getEntityAttribute(field_110186_bp).applyModifier(
					new AttributeModifier("Leader aeryacursed bonus", this.rand.nextDouble() * 0.25D + 0.5D, 0));
			this.getEntityAttribute(SharedMonsterAttributes.maxHealth).applyModifier(
					new AttributeModifier("Leader aeryacursed bonus", this.rand.nextDouble() * 3.0D + 1.0D, 2));
		}
	...
	public boolean interact(EntityPlayer p_70085_1_) {
		...
		if (var2 != null && var2.getItem() == Items.golden_apple && var2.getMetadata() == 0
	...
	public void handleHealthUpdate(byte p_70103_1_) {
		if (p_70103_1_ == 16) {
			if (!this.isSlient()) {
				this.worldObj.playSound(this.posX + 0.5D, this.posY + 0.5D, this.posZ + 0.5D, "mob.aeryacursed.remedy",
	...
	public void onDeath(DamageSource cause) {
		...
		if (cause.getEntity() instanceof EntityCreeper && ((EntityCreeper) cause.getEntity()).getPowered()
